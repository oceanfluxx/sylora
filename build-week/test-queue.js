...
async function main(){
  ...
  try{
    ...
    child=spawn(process.execPath,[path.join(__dirname,'server.js')],{env:{...process.env,PORT:String(port),RPC_URL:rpc,REGISTRY_ADDRESS:await reg.getAddress(),TOKEN_ADDRESS:await token.getAddress(),SYLORA_DATA_DIR:tmp},stdio:'ignore',windowsHide:true});
    ...
    assert.equal((await post('tree_planting','Duplicate')).status,429,'rate limited');
    await wait(5100);
    ...
    const second=await post('recycle','Dropped off recycling');assert.equal(second.status,201);
